...
module.exports = function ProxyMenu(mod) {
	...
	const commands = {
		...
		premium: () => {
			...
			mod.command.message(`Add. menu icons on the premium panel: ${mod.settings.premiumSlotEnabled ? "Enabled" : "Disabled"}`);
		},
		lobby: () => {
		...
		build: () => {
			...
			mod.command.message(`Signs. Christmas trees. Calendar: ${mod.settings.spawnBuild ? "Hidden" : "Shown"}`);
		},
		scene: () => {
		...
		tolobby: () => {
			...
			mod.command.message(`Quick relog on characters: ${mod.settings.lobby ? "Enabled" : "Disabled"}`);
		},
		drunk: () => {
			mod.settings.drunk = !mod.settings.drunk;
			mod.command.message(`Hide Drunk Screen: ${mod.settings.drunk ? "Hidden" : "Shown"}`);
		},
		brooch: () => {
			mod.settings.brooch = !mod.settings.brooch;
			mod.command.message(`Hiding the brooch animation: ${mod.settings.brooch ? "Hidden" : "Shown"}`);
		},
		dbe: () => {
			mod.settings.backstep = !mod.settings.backstep;
			mod.command.message(`Auto reset evaid (priest): ${mod.settings.backstep ? "Enabled" : "Disabled"}`);
		},
		ggreset: () => {
			mod.settings.ggreset = !mod.settings.ggreset;
			mod.command.message(`Ghillieglade auto reset after using teleport scroll: ${mod.settings.ggreset ? "Enabled" : "Disabled"}`);
		},
		autoaccept: () => {
		...
		autoreset: () => {
			...
			mod.command.message(`Auto accept reset/disband party: ${mod.settings.autoreset ? "Enabled" : "Disabled"}`);
		},
		hotkey: arg => {
			if (!arg) {
		...
		sla: percent => {
			...
			if (percent <= 0) {
				return mod.command.message("Low HP");
			}
			dropHp(percent);
		},
		backwalk: () => {
			mod.settings.backwalk = !mod.settings.backwalk;
			mod.command.message(`Walking backwards (only visible to others) : ${mod.settings.backwalk ? "Enabled" : "Disabled"}`);
		},
		circlewalk: () => {
			mod.settings.circlewalk = !mod.settings.circlewalk;
			mod.command.message(`Walking while spinning (only visible to others) : ${mod.settings.circlewalk ? "Enabled" : "Disabled"}`);
		},
		circle: arg => {
			mod.settings.circle = arg;
			mod.command.message(`Turning coefficient <font color="#5da8ce">${arg}</font>`);
		},
		autobox: () => {
			mod.settings.openbox = !mod.settings.openbox;
			mod.command.message(`Open boxes in one click: ${mod.settings.openbox ? "Enabled" : "Disabled"}`);
		},
		boxdelay: (arg) => {
			if (arg === "0") {
				mod.settings.boxdelay = 0;
				mod.command.message("The delay in opening boxes has been removed");
			} else if (!isNaN(arg)) {
				mod.settings.boxdelay = parseInt(arg);
				mod.command.message(`Delayed opening: ${ mod.settings.boxdelay / 1000 } сек`);
			}
		}
	};
		...
		if (!opening) {
			mod.hook("S_GACHA_START", "*", () => {
				...
				if (mod.game.inventory.getTotalAmount(gachaId) >= 5) {
					mod.command.message("Opening. To stop, click on the item again..");
				}
				return false;
			});
		} else {
			opening = false;
			mod.command.message("Stop.");
		}
	});
		...
		if (opening && mod.game.inventory.getTotalAmount(id) >= 1) {
			...
			mod.hookOnce("S_CANCEL_CONTRACT", "*", () => {
				...
				if (mod.game.inventory.getTotalAmount(id) < 1) {
					mod.command.message("Finished the opening.");
				}
			});
		}
	}
